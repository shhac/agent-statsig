...
Releasing `agent-statsig` is automated. Pushing a `v*` tag triggers
...
and regenerate + push `Formula/agent-statsig.rb` (with shell completions) to the tap.
The tag also triggers `.github/workflows/publish-skill.yml`, which publishes
`skills/agent-statsig` to `shhac/agent-skills`. **No manual build, and no manual
formula bump.**
...
5. Verify CI and the outputs:
   ```bash
   gh run watch --repo shhac/agent-statsig          # release + Publish skill runs green
   gh release view "v${new_version}" --repo shhac/agent-statsig   # 6 assets
   ```
   Install / upgrade: `brew install shhac/tap/agent-statsig` · `brew upgrade shhac/tap/agent-statsig`

## Manual fallback (only if the workflow itself is broken)

Re-run a failed release with `gh run rerun <id> --repo shhac/agent-statsig`. To bypass
...
`Formula/agent-statsig.rb` by hand (see this file's git history for the old full flow).
...
repo-specific "agent-statsig release automation (env-scoped)" one). The skill
...
```bash
ssh-keygen -t ed25519 -N "" -C "agent-statsig release automation" -f tap_key
gh repo deploy-key add tap_key.pub -R shhac/homebrew-tap --allow-write \
  --title "agent-statsig release automation (env-scoped)"
gh secret set TAP_DEPLOY_KEY --repo shhac/agent-statsig --env homebrew-tap < tap_key
