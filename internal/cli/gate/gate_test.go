...
func TestMergeConditionValues(t *testing.T) {
	...
	result := MergeConditionValues(existing, []string{"c@test.com"}, nil)
	if len(result) != 3 {
		t.Fatalf("expected 3, got %d", len(result))
	}

	result = MergeConditionValues([]string{"a@test.com", "b@test.com"}, []string{"a@test.com"}, nil)
	if len(result) != 2 {
		t.Errorf("duplicate should not be added, got %d", len(result))
	}

	result = MergeConditionValues([]string{"a", "b", "c"}, nil, []string{"b"})
	if len(result) != 2 || shared.SliceContains(result, "b") {
		t.Errorf("b should be removed: %v", result)
	}

	result = MergeConditionValues([]string{"a"}, []string{"b", "c"}, []string{"a"})
