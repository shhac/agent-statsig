...
func TestMoveRule(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := MoveRule(namedRules("a", "b", "c"), tc.idOrName, tc.position)
			if err != nil {
...
func TestMoveRuleErrors(t *testing.T) {
	...
	if _, err := MoveRule(rules, "missing", "top"); err == nil {
		t.Error("unknown rule should error")
	}
	if _, err := MoveRule(rules, "a", "0"); err == nil {
		t.Error("position 0 should error (1-based)")
	}
	if _, err := MoveRule(rules, "a", "4"); err == nil {
		t.Error("out-of-range position should error")
	}

	dup := append(rules, api.Rule{ID: "id-x", Name: "a"})
	if _, err := MoveRule(dup, "a", "top"); err == nil {
		t.Error("ambiguous name should error")
	}
	if got, err := MoveRule(dup, "id-x", "top"); err != nil || got[0].ID != "id-x" {
